...
import type { ArcadeSettings, BacklogBomb, Enemy, GameState, Hazard } from "./types.ts";
...
export function renderArcade(
  ...
  drawBackfillWallWarning(context, state, width, height);
  drawBacklogFirewallWarning(context, state, width, height);
  ...
  for (const enemy of state.enemies) {
    const hiddenForBacklogIntermission = enemy.bossKind === "backlog" &&
      (state.backlogHits === 2 || state.backlogHits === 3) &&
      state.backlogIntermissionStage === 2;
    if (hiddenForBacklogIntermission) continue;
    drawEnemy(context, enemy, state.elapsed, state.activePowerups.freezeFor > 0);
  }
  drawBacklogTiles(context, state);
  for (const bomb of state.backlogBombs) drawBacklogBomb(context, state, bomb);
  drawBacklogIntegrity(context, state, width);
...
function drawProjectile(
  ...
  if (projectile.pattern === "backfill-wall" || projectile.pattern === "backlog-firewall") {
    ...
    context.fillStyle = backlog ? "#f3a65a" : "#ff334f";
    context.strokeStyle = backlog ? "#ffe0a3" : "#ff9cab";
    context.shadowColor = backlog ? "#f3a65a" : "#ff334f";
...
function drawBossDialogue(
  ...
  context.save();
  const interactiveBacklog = state.phase === "boss" &&
    ACTS[state.actIndex]?.boss.kind === "backlog" &&
    state.backlogIntroStage < 5;
  const danger = dialogue.tone === "danger";
  const lineHeight = danger ? 22 : 16;
  context.font = danger ? "900 17px ui-monospace, monospace" : "700 11px ui-monospace, monospace";
  const lines = wrapText(context, dialogue.text, danger ? 290 : 260);
  const elapsed = dialogue.maxLife - dialogue.life;
  const visibleText = dialogue.revealRate
    ? dialogue.text.slice(0, Math.floor(elapsed * dialogue.revealRate))
    : dialogue.text;
  const visibleLines = wrapText(context, visibleText, danger ? 290 : 260);
  const bubbleWidth = Math.min(
    danger ? 310 : 280,
    Math.max(170, ...lines.map((line) => context.measureText(line).width + 28)),
  );
  const textHeight = 24 + lines.length * lineHeight;
  const bubbleHeight = textHeight + (interactiveBacklog ? 19 : 0);
  ...
  context.strokeStyle = danger ? "#ff334f" : "#70dff2";
  context.shadowColor = danger ? "#ff334f" : "transparent";
  context.shadowBlur = danger ? 16 : 0;
  context.lineWidth = danger ? 3 : 2;
  context.beginPath();
  context.roundRect(x - bubbleWidth / 2, y, bubbleWidth, bubbleHeight, 8);
  context.fill();
  context.stroke();
  context.shadowBlur = 0;
  context.fillStyle = danger ? "#ff5b74" : "#e7ffff";
  context.textAlign = "center";
  context.textBaseline = "top";
  const backlogReveal = interactiveBacklog && state.backlogIntroStage === 3;
  visibleLines.forEach((line, index) => {
    context.fillStyle = backlogReveal && line.toLowerCase().startsWith("and absolutely permanent")
      ? "#ff5b74"
      : danger
      ? "#ff5b74"
      : "#e7ffff";
    context.fillText(line, x, y + 12 + index * lineHeight);
  });
  if (interactiveBacklog) {
    const promptY = y + textHeight;
    context.strokeStyle = danger ? "rgba(255, 91, 116, 0.44)" : "rgba(112, 223, 242, 0.36)";
    context.lineWidth = 1;
    context.beginPath();
    context.moveTo(x - bubbleWidth / 2 + 12, promptY);
    context.lineTo(x + bubbleWidth / 2 - 12, promptY);
    context.stroke();
    context.fillStyle = "rgba(248, 212, 119, 0.88)";
    context.font = "900 8px ui-monospace, monospace";
    context.textBaseline = "top";
    context.fillText("CLICK / SPACE TO CONTINUE", x, promptY + 5);
  }
  context.restore();
}

function drawBacklogTiles(
  context: CanvasRenderingContext2D,
  state: GameState,
) {
...
function drawBacklogFirewallWarning(
  context: CanvasRenderingContext2D,
  state: GameState,
  width: number,
  height: number,
...
function drawBackfillWallWarning(
  context: CanvasRenderingContext2D,
  state: GameState,
  width: number,
  height: number,
) {
  if (state.backfillWallWarningFor <= 0) return;
  const pulse = 0.35 + Math.sin(state.elapsed * 18) * 0.15;
  const direction = state.backfillWallDirection;
  context.save();
  context.fillStyle = `rgba(255, 51, 79, ${pulse})`;
  context.strokeStyle = "#ff5b74";
  context.lineWidth = 3;
  context.setLineDash([10, 6]);
  if (direction === 0 || direction === 2) {
    const y = direction === 0 ? 0 : height - 18;
    context.fillRect(0, y, width, 18);
    context.beginPath();
    context.moveTo(0, direction === 0 ? 20 : height - 20);
    context.lineTo(width, direction === 0 ? 20 : height - 20);
    context.stroke();
  } else {
    const x = direction === 3 ? 0 : width - 18;
    context.fillRect(x, 0, 18, height);
    context.beginPath();
    context.moveTo(direction === 3 ? 20 : width - 20, 0);
    context.lineTo(direction === 3 ? 20 : width - 20, height);
    context.stroke();
  }
  context.restore();
}
...
function drawPowerupDrops(context: CanvasRenderingContext2D, state: GameState) {
  for (const drop of state.powerupDrops) {
    ...
    if (drop.fallSpeed) {
      ...
      context.font = "900 8px ui-monospace, monospace";
      context.fillText("▼", 0, drop.radius + 10);
    }
    context.restore();
  }
}
